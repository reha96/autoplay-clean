...
We conducted a two-day online lab experiment to examine how autoplay feature affects time allocation between transcription of random characters and watching funny animal videos. The multi-day structure allowed us to measure the gap between planned and actual behavior, . Participants completed two separate data collection sessions with a cooling-off period of 24 hours (up to 48 hours) between sessions.
...
\footnotesize
\item[] \textit{Note:} This table shows the experimental components administered across two data collection waves, with columns ordered chronologically within each day. The first block established baseline effects, while the second block incorporated the Multiple Price List to measure demand for a commitment device. Decision horizon refers to the time between planned time allocation and the main session.
\end{tablenotes}
\label{summary_table}
\end{threeparttable}
\end{table}
...
This study examined whether autoplay features in isolation can override users' stated preferences for media consumption. In a two-day online experiment with 236 participants, we find no evidence that autoplay increases video consumption when content is held constant. Participants in our setting viewed autoplay positively, exhibiting willingness-to-pay for the convenience it provides. However, systematic deviations from stated preferences suggest experimenter demand effects influenced behavior, with participants transcribing more than planned.

Our null findings reveal a fundamental challenge in studying social media interface features: they are hard to evaluate in isolation from their embedded context. Autoplay's effectiveness appears to depend on the personalized, engaging content it delivers. Users are not drawn to endless streams of educational documentaries, but to drama, gossip, and emotionally compelling material that algorithms curate personally for them. In our controlled setting with identical and manually curated videos, we may have stripped away the very element that makes autoplay change behavior: its coupling with personalized, addictive content.

These findings have important implications for research on interface design and digital well-being. Laboratory studies like ours that attempt to isolate the behavioral effects of interface features may misrepresent their real-world impacts. The monetary incentives and task framing necessary for internal validity may inadvertently create demand effects that confound the very phenomena researchers seek to study. While autoplay shows no effect in our experimental setting, it seems to have stronger impacts in leisure contexts \citep{hiniker2018coco,schaffner2025pahi}.

For policymakers considering regulations on autoplay features, our findings suggest that regulating interface features alone may be insufficient if the underlying content and algorithmic curation remain unchanged. Effective digital well-being interventions may need to address the entire ecosystem of personalized, engaging content delivery rather than focusing solely on individual design elements. Future research should prioritize developing experimental methods that better approximate genuine media consumption environments in the field (see e.g., \citet{beknazar2025toxic,purohit2023starving}) while maintaining the control necessary for causal inference.
